...
impl IndentParenExprArgs {
    pub fn filter_out_parenthesized_tokens(expression_tokens: TreeElementTokenIterator) -> Vec<Token> {
        ...
        for token in expression_tokens {
            match token.kind {
                ...
                _ => { if paren_depth == 0 { token_list.push(token); }
                }
            }
        }
        token_list
    ...
    pub fn from_for(node: &ForContent) -> Option<IndentParenExprArgs> {
        ...
        filtered_member_ranges.extend(&mut Self::filter_out_parenthesized_tokens(node.pre.tokens()).into_iter().map(|t| t.range));
        filtered_member_ranges.push(node.lsemi.range());
        filtered_member_ranges.extend(&mut Self::filter_out_parenthesized_tokens(node.cond.tokens()).into_iter().map(|t| t.range));
        filtered_member_ranges.push(node.rsemi.range());
        filtered_member_ranges.extend(&mut Self::filter_out_parenthesized_tokens(node.post.tokens()).into_iter().map(|t| t.range));
    ...
    pub fn from_foreach(node: &ForeachContent) -> Option<IndentParenExprArgs> {
        Some(IndentParenExprArgs {
            members_ranges: Self::filter_out_parenthesized_tokens(node.expression.tokens()).into_iter().map(|t| t.range).collect(),
            lparen: node.lparen.range(),
        })
    }
    ...
    pub fn from_function_call(node: &FunctionCallContent) -> Option<IndentParenExprArgs> {
        ...
        for (arg, _comma) in node.arguments.iter() {
            filtered_member_ranges.extend(Self::filter_out_parenthesized_tokens(arg.tokens()).into_iter().map(|t| t.range));
        }
        Some(IndentParenExprArgs {
            members_ranges: filtered_member_ranges,
            lparen: node.lparen.range(),
    ...
    pub fn from_paren_expression(node: &ParenExpressionContent)
        ...
        Some(IndentParenExprArgs {
            members_ranges: Self::filter_out_parenthesized_tokens(node.expr.tokens()).into_iter().map(|t| t.range).collect(),
            lparen: node.lparen.range(),
        })
    }
    ...
    pub fn from_while(node: &WhileContent) -> Option<IndentParenExprArgs> {
        Some(IndentParenExprArgs {
            members_ranges: Self::filter_out_parenthesized_tokens(node.cond.tokens()).into_iter().map(|t| t.range).collect(),
            lparen: node.lparen.range(),
        })
    }
    ...
    pub fn from_cast(node: &CastContent) -> Option<IndentParenExprArgs> {
        ...
        Some(IndentParenExprArgs {
            members_ranges: Self::filter_out_parenthesized_tokens(cast_member_tokens).into_iter().map(|t| t.range).collect(),
            lparen: node.lparen.range(),
        })
    }
...
impl IndentContinuationLineRule {
    ...
    pub fn check<'a> (&self, acc: &mut Vec<DMLStyleError>,
        ...
        for token in args.token_list {
            match token.kind {
                TokenKind::RParen => {
                    if Some(token.range.row_start.0) != last_row {
                        last_row = Some(token.range.row_start.0);
                    }
                }
                _ => {
                    if Some(token.range.row_start.0) != last_row {
